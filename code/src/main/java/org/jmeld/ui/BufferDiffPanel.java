...
public class BufferDiffPanel extends AbstractContentPanel implements ConfigurationListenerIF {
    ...
    public void diff(String path1, int start1, int end1, String path2, int start2, int end2){
    	String file1;
    	String file2;
		try {
			file1 = FileUtils.readFileToString(path1);
			file2 = FileUtils.readFileToString(path2);
	        currentRevision = diff.diff(file1.split(System.lineSeparator()), file2.split(System.lineSeparator()),
	            		start1, end1, start2, end2, getDiffNode().getIgnore());
